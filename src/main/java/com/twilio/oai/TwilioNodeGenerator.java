...
public class TwilioNodeGenerator extends TypeScriptNodeClientCodegen {
    ...
    public void processOpenAPI(final OpenAPI openAPI) {
        ...
        openAPI.getPaths().forEach((name, path) -> path.readOperations().forEach(operation -> {
            ...
            final String tag = cleanPath(name).replace("/", PATH_SEPARATOR_PLACEHOLDER);
            ...
            operation.addExtension("x-dependents",
                                   openAPI
                                       ...
                                       .filter(p -> removePathParamIds(p)
                                           .matches(escapeRegex(removePathParamIds(name)) + "/[^{]+"))
    ...
    private String getResourceName(final String path) {
        return getLastPathPart(cleanPath(path));
    }

    private String getLastPathPart(final String path) {
    ...
    private String cleanPath(final String path) {
        return path.replaceFirst("/[^/]+/", "") // Drop the version
            .replaceAll("/\\{[^}]+}", ""); // Drop every path parameter.
    }

    private String removePathParamIds(final String path) {
